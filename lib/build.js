...
mem.on('message', function(ch, val){
  if(ch == "load_mp"){
    ...
    if(doc && dc){
      var docid = val._id
      mem.publish("stop_all_container_obs", [docid], function(err){
        setTimeout(function(){
          if(!err){
                          ...
                          return function(){
                            ...
                            if(_.isObject(def_cont)){
                              if(def_cont.Take && def_cont.From){
                                ...
                                request.exec(con, false, s, function(db_cont){
                                  if(_.isObject(db_cont) && db_cont.Definition){
                                    ...
                                    utils.replace_all(db_cont, def_cont.Replace, function(repl_cont){
                                      build_container(path_c, repl_cont, function(res){
                                        if(res.ok){
                                          mem.publish("start_container_obs", [docid, j], function(err){
                                            if(!err){
                                              log.info(ok
                                ...
                                build_container(path_c, def_cont, function(res){
                                  if(res.ok){
                                    mem.publish("start_container_obs", [docid, j], function(err){
                                      if(!err){
                                        log.info(ok
            ...
            log.error({error:err}
                     , "error on attempt to publish stop_container_obs: " + docid);
          }
        }, 2 * deflt.container.heartbeat);
      }); // publish stop_container_obs
    }else{
      log.error({error:"no doc or container"}
                  ...
                  mem.set(path.concat(["definition"]), container["Definition"] || [[{}]], function(err){
                    if(!err){
                      ...
                      mem.get(path.concat(["definition"]), function(err, definition){
                        ...
                        utils.cp( definition , cstr.ready, function(defcp){
