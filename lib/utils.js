...
/**
 * Macht aus dem geschachtelten State Array ein
 * flaches Array
 * @param {Array} s state array
 * @return {Array} a flat array
 */
var as_arr = function(s){
  if(_.isArray(s) && _.isArray(s[0])){
 ...
 * @param {Array} a state array
 * @return {Bool} alle auf val
 */
var all_same =  function(arr, val){
  if(_.isArray(arr) && _.isString(val)){
    _.every(arr, function(i){
      return i == val;
    });
  }else{
    return false;
 ...
 * @param {Object} dataset Datenobjekten
...
var data_to_doc = function(doc, path, data, cb){
  ...
  if(data                   &&
    ...
    var Nds     =  data.Result.length;
    log.info(ok
            , "found "+ Nds+" data sets to store" );
    for(var i = 0; i < Nds; i++){

      var  dv =  op.get(doc, path)
      ...
      if(dat.Type  &&
        ...
        op.set(doc, [path, pos, "Type"].join("."), dat.Type);
        op.set(doc, [path, pos, "Unit"].join("."), dat.Unit);
        ...
        op.set(doc, [path, pos, "Value", ppos].join("."), dat.Value);
        ...
        if(dat.Comment){
          ...
          op.set(doc, [path, pos, "Comment", ppos].join("."), dat.Comment);

        }
      }else{
        for(var j in dat){
          // write stuff like Gas, okpk ...
          op.set(doc, [path, j].join("."), dat[j]);
        }
      }
