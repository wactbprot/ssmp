...
var write_to_exchange = function(mp, task, data, cb){
  ...
  if(fromdata){
    var kpath  = _.keys(data.ToExchange);

    if(kpath.length === 1){
    log.info({ok:true}, "found key");
      // why _.first?
      ...
      log.error({error:"data loss"},
                "data.ToExchange contains more than 1 key");
      if(_.isFunction(cb)){
        cb("error");
      }
    }
