 ...
 */
var data_to_doc = function(doc, path, datares, cb){

  if(datares && datares.Result){

    var dmod    = gen.mod(doc),
        dataset = datares.Result,
        Nds     = dataset.length;
  ...
  if( task &&
    ...
    if(data.Result            &&
      ...
      for(var i = 0; i < Nid; i++){
        ...
        request.exec(mp,  task, path, rdcon, false, data, incertcb)
      }
    }else{
      ro = {error:"can not query doc with given data"};
      log.error(ro,
    ...
    if(_.isFunction(cb)){
      cb({ok:true , warn: "empty Id array"});
    }
  }
  }else{
    ro = {error:"no task"};
