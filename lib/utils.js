  ...
  if(data_result && _.isArray(data_result) && data_result.length > 0 ){
    ...
    for(var i = 0; i < Nds; i++){
      ...
      if(_.isString(data_struct.Type)){ // write Type, Value, Unit- Structures
        ...
        if((_.isNull(data_struct.Value) || _.isNumber(data_struct.Value) || _.isString(data_struct.Value)) && _.isString(data_struct.Unit)){
          ...
          // ----- NValue
          if(data_struct.NValue){
            var doc_struct_nvalue = op.get(doc, [path, pos, "NValue"].join("."))
            if(!_.isArray(doc_struct_nvalue)){
              op.set(doc, [path, pos, "NValue"].join("."), []);
            }
            // ppos wird von value genommen
            op.set(doc, [path, pos, "NValue", ppos].join("."), data_struct.NValue);
          }
