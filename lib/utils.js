...
var query_cd = function(mp, task, data, cb){
  ...
  if(task.Id         &&
    ...
    if(data.Result            &&
      ...
      for(var i = 0; i < Nid; i++){
        var id     = Id[i],
            wrtcon = net.wrtdoc(mp,id);

        var incertcb = function(j){
        ...
        var getcb = function(j){
                      return function(error, doc){
                        ...
                        if(doc){
                          ...
                          //  dbcon.insert(doc, incertcb);
                            
                            request(mp,  task, path, wrtcon, doc, incertcb)
                          });
                        }; // if doc
                      }
                    }(i)

        dbcon.get(id, getcb);// db.get
