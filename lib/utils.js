...
var get = function(mps, req, cb){
  var msg, ro, obj, path,
      id     = req.params.id,
      struct = req.params.struct;
  if(id && mps[id]){
    if(struct  && mps[id][struct] && _.isFunction(mps[id][struct].get)){
      mps[id][struct].get(get_path(req), function(obj){
    ...
    if(task.Id         &&
      ...
      if(task.DocPath){
        ...
        for(var i = 0; i < Nid; i++){
          ...
          request.exec(mp, rdcon, task, path, false
                      ,function(last){
                         return function(doc){
                           log.info({ok:true},
                           ...
                           data_to_doc(doc, docpath, data
                                      , function(filled_doc){
                                          ...
                                          // (mp, con, task, path, wrtdata, cb)
                                          request.exec(mp, wrtcon, task, path, JSON.stringify(filled_doc)
                                                      , function(res){
                                                          if(res.error){
                                                            log.error(res
