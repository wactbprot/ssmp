  ...
  if(state === ctrlstr.ready){
    mp.state.set(path, ctrlstr.work, function(){
      ...
      if(_.isObject(task) && task.Action){
        ...
        if(task.Replace){
          if(_.isArray(task.Replace)){

            for( var l = 0; l < task.Replace.length; l++){
              var token = task.Replace[l],
                  value = mp.exchange.get([token, "Value"]);
              if(token && value){
        ...
        if(task.StopIf){
          if(mp.exchange.get([task.RunIf, "Value"])){
            ok = false;
          }else{
            ok = true;
          }
