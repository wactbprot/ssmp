 ...
 log.info({ok:true}, "build container " + pos);
  mp.element.set([pos], container["Element"], function(){
    mp.title.set([pos], container["Title"], function(){
      buildDefState(mp, pos, container["Recipe"], cb);
    });
  });
};
...
var buildDefState = function(mp, pos, recipe, cb){
  log.info({ok:true}, "sync def and state" + pos);
  mp.definition.set([pos], recipe, function(){
    mp.state.set([pos], clone(recipe), function(){
...
exports.buildDefState = buildDefState;
