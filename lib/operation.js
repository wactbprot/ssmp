  ...
  if(state === ctrlstr.ready){
    mp.state.set(path, ctrlstr.work, function(){
      ...
      if(_.isObject(task) && task.Action){
        var ok     = true,
            statestr  = ctrlstr.exec,

            oktxt  = "task " + task.TaskName + " executed",
            errtxt = "task " + task.TaskName + "task execution failed";
        ...
        if(task.StopIf){
          if(mp.exchange.get(task.StopIf.split(".")) == "true"){
            ok = false;
            statestr = ctrlstr.exec;
            oktxt = "task " + task.TaskName + " ends";
          }else{
            ok = true;
            statestr = ctrlstr.ready;
            oktxt = "task " + task.TaskName + " waiting for task.StopIf to become true";
          }
        }
        if(task.RunIf){
          if(mp.exchange.get(task.RunIf.split(".")) == "true"){
            ok = true;
            statestr = ctrlstr.exec;
            oktxt = "task " + task.TaskName + " started";
          }else{
            ok = false;
            statestr = ctrlstr.ready;
            oktxt = "task " + task.TaskName + " awaiting data from exchange";
          }
        }
        ...
        if(_.isFunction(worker[exec])){
          ...
          if(ok){
            worker[exec](mp, task, path, function(res){
              if(res === "ok"){
                mp.state.set(path, statestr, function(){
                  log.info(oktxt)
                })
              } // return ok
              if(res === "error"){
...
var ini = function(mps, req, cb){
  ...
  if(typeof rb === "object"){
    ...
    utils.build(mp, rb, cb);
  }
  if(id === sim){
    log.info({ok:true},"request the md simulation")
    utils.build(mp, simjs, cb);
  }
