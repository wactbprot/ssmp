 ...
 * mit dem so aufgefllten Dokument auf.
 *
 * @method data_to_doc
 ...
 */
module.exports = function (doc, path, result, cb){

  if(doc && path && _.isArray(result) && !_.isEmpty(result)){
    var Nds     = result.length;
    log.trace(ok
    ...
    log.error(err
             , "the test on doc, path or data failed");
    cb(err);
  }
};
