    ...
    mem.set(epath.concat(["start_time", "Value", "value"]), stime, function(err){
      mem.set(epath.concat(["start_time", "Unit", "value"]), "ms", function(err){
        mem.set(epath.concat(["run_time", "Unit", "value"]), "ms", function(err){
          ...
          timerId[mpid] = setInterval( //
            function(){
              var rtime = (new Date()).getTime() - stime
              mem.get(epath, function(err,val){
                console.log(val );
                mem.set(epath.concat(["run_time", "Value", "value"]), rtime, function(err){
                  ...
                  mem.get(path.concat([i, "ctrl"]), function(no){
                                                      return function(err, cmdstr){
                                                        ...
                                                        mem.get(spath, function(err, state_no){
                                                          ...
                                                          if(cmd == cstr.run){
                                                            mem.set(cpath, "runing", function(){
                                                              mem.publish("run", nopath, function(err){
                                                                if(!err){
                                                                  log.info(ok
                                                                          ,"run event published");
                                                                }else{
                                                                  log.info({error:err}
                                                                          ,"error on publishing run event");
                                                                }
                                                              });
                                                            }); // set ctrl loading
                                                          } // load
