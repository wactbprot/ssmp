...
var observe = function (ch, path){
  ...
  if(ch == "executed" || ch == "stop"){
    ...
    mem.get([mpid, no, "recipe"], function (err, recipe){
      if(!err){
        utils.cp(recipe , cstr.ready, function(recipecp){

          mem.set(path.concat(["state"]), recipecp, function(err){
  ...
  if(ch == "stop_container_obs" || ch == "stop_all_container_obs"){
    stop_cont(path, ch, timerId, function(err, path){
      if(!err){
        log.info(ok
                , "clear interval on event stop_container_obs for path:  " + path.join(" "));
      }else{
        log.error({error:err}
                 , "error on attempt to clear interval on event stop_container_obs for path:  " + path.join(" "))
      }
    });
...
var stop_cont = function(path, ch, timerId, cb){
  ...
  if(path && _.isArray(path) && path.length >= 2){
    ...
    if(timerId[mpid] && _.isObject(timerId[mpid])){
      ...
      if(_.isFunction(cb)){
        cb("no or wrong timerId object", path)
      }
    }
  }else{// path
    if(_.isFunction(cb)){
