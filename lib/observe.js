/**
 * Observes the mp definitions; starting and stoping of containers
 *
 * @module observe
 */
            ...
            timerId[mpid][no] = setInterval(function (){
                                  time_to_exchange(path, false, function(err, path){
                                    if(!err){
                                      ...
                                      log.error(err
                                                 , "error on time to exchange");
                                    if(_.isFunction(cb)){
                                      cb(err);
                                    }
                                    }
                                  }); // time
                                }, defaults.system.heartbeat); // intervall
          }); // get defaults
        }else{ // timerId ae
          log.trace(ok
  ...
  if(path && _.isArray(path) && path.length >= 1){
    ...
    if(first){
      ...
      mem.get([mpid, "exchange","start_time"], function(err, res){
        ...
        var val = {Value: Math.round(ct*1000)/1000,
                   Unit:"min"}
