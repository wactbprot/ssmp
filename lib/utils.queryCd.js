 ...
 * zum  Abspeichern des nun aufgefüllten cd.
 *
 * @method query_cd
    ...
    if(task.Id && task.Id.length  > 0){
      if(data && data.Result && data.Result.length >0){
        if(task.DocPath){
          ...
          err = new Error("with given task object");
          log.error(err
        ...
        err = new Error("with given data object");
        log.error(err
                 , "test on task.result failed");
        if(_.isFunction (cb)){
          cb(err);
        }
      }
...
var request_cd = function(task, id, data, last, cb){
  net.rddoc(id, function(err,rcon){
    request.exec(rcon, task, false, function (err, doc){
      if(err){
        if(_.isFunction (cb)){
          cb(err);
        }
        ...
        data_to_doc(doc, task.DocPath, data.Result
                   , function (err, doc){
                       if(!err && doc._id){
                         log.trace(ok
                                  ,"doc filled up with date try to store back");
                         net.wrtdoc(id, function(err, wcon){
                           request.exec(wcon, task, JSON.stringify(doc), function (err, res){
                             if(err){
