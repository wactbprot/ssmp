...
(function() {
  ...
  server.put("/:id/go/:container",
             function(req, res, next) {
               ...
               req.params[0] = "/" + id + "/go/" + container;
               api.set(mps, req);
               if(typeof go === "boolean" && go){
                 console.log("goooooooooooo");
                 //res.send(ctr.run(id, container));
               }else{
                 console.log("stoooooooooop");
                 //res.send(ctr.stop(id, container));
               }
               return next();
             });
