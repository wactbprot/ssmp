  ...
  if(task.FromExchange){
    if(_.isArray(task.FromExchange)){
      ...
      for( var l = 0; l < N; l++){
        ...
        mem.get(path.concate(path_l), function(last){
      ...
      mem.get(path.concate(path_l), function(err, value){
        if(!err){
...
var task_stop_if = function(path, task, ok, cmdstr, cb){
  ...
  if(task.StopIf){
    ...
    mem.get(path.concate(path_s), function(err, value){
      if(!err){
...
var task_run_if = function(path, task, ok, cmdstr, cb){
  ...
  if(task.RunIf){
    ...
    mem.get(path.concate(path_r), function(err, value){
      if(!err){
        if(value == "true"){
          ok     = true;
          cmdstr = cstr.exec;
