...
var run = function (path, s, p, task){
  ...
  if(_.isObject(task) && task.Action && _.isFunction (worker[task.Action])){
    ...
    exchange_replace(path_e, task, ok, cmdstr, function (task, ok, cmdstr){
      run_if(path_e, task, ok, cmdstr, function (task, ok, cmdstr){
        stop_if(path_e, task, ok, cmdstr, function (task, ok, cmdstr){
  ...
  if(task.FromExchange && _.isObject(task.FromExchange)){
    ...
    for( var l = 0; l < N; l++){
      ...
      mem.get(path.concat(path_l), function (m, n){
					     ...
					     log.info({ok:true}
						      , "replaced "+ value + " in path: " + pathes[m]);
                                         }else{
                                             ok = false;
