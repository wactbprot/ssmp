...
var run = function (path, s, p, task){
  ...
  if(_.isObject(task) && task.Action && _.isFunction (worker[task.Action])){
    ...
    task_exchange(path_e, task, ok, cmdstr, function (task, ok, cmdstr){
      task_run_if(path_e, task, ok, cmdstr, function (task, ok, cmdstr){
        task_stop_if(path_e, task, ok, cmdstr, function (task, ok, cmdstr){
