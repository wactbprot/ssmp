    ...
    for(seqDef = 0; seqDef < NseqDef; seqDef++){
      ...
      for(parDef = 0; parDef < NparDef; parDef++){
        ...
        if(defStep.Customer){
          ...
          var nArr = expand_cust(mp, clone(defStep), calibobjs)
...
var insert_par = function(mp, def, calibobjs, cb){
  mp.meta.get([], function(meta){
    ...
    for(seqDef = 0; seqDef < NseqDef; seqDef++){
      var NparDef = def[seqDef].length;
      for(parDef = 0; parDef < NparDef; parDef++){
        var defStep = ndef[seqDef][parDef];

        if(defStep.ExpandPar){
          ndef[seqDef].splice(parDef, 1)
          ndef[seqDef] =  _.union(def[seqDef], expand_par(mp, clone(defStep), calibobjs));
        } // expand par
      } // for parDef
    } // for seqDef
    ...
    for(seqDef = 0; seqDef < NseqDef; seqDef++){
      ...
      for(parDef = 0; parDef < NparDef; parDef++){
        ...
        if(defStep.ExpandSeq){
          var seqArr = expand_seq(mp, clone(defStep), calibobjs)
...
var expand_cust = function(mp, nParStep, calibobjs){
  ...
  if(cdIds.length > 0){
    for(var i = 0; i < NcdIds; i++){
      ...
      var cps = clone(nParStep)
      cps.TaskName = deviceName + "-" + nParStep.TaskName;
      nParArr.push(cps)
    }
  }else{

    nParStep.TaskName = deflt.custDevPrefix + "-" + nParStep.TaskName
    nParArr.push(nParStep)
  }
