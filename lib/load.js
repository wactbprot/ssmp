...
var all = function(path){
  ...
  mem.get([path[0]].concat(["id"]), function(err, calibobjs){
        ...
        mem.remove(path.concat(["recipe"]), function(err){
          if(!err){
            mem.get(path.concat(["definition"]), function(err, def){
              insert(path, def, calibobjs, function(def){
                distribute_def(path, def)
              });
            }); // definition
          } // if res ok recipe to []
        }); // set recipe to []
...
var distribute_def = function(path, def, cb){
  ...
  mem.get([path[0]].concat(["meta"]), function(err, meta){
    mem.set(path.concat(["state"]), "loading", function(err){
...
var fetchtask = function(nopath, subpath,  task){
  var con      = net.task()
  ...
  request.exec(con, task, strtask, function(task){
    var spath = nopath.concat(["state"]).concat(subpath)
      , rpath = nopath.concat(["recipe"]).concat(subpath)
    if(task.error){
