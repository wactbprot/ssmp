...
var load = function(mp, path){
  ...
  var state    = mp.state.get(path),
      calibobjs     = mp.id.get(),
      calibids      = _.keys(calibobjs); // calibration doc ids

  log.info({ok:true, fn: "load"}, "try to load path: " + JSON.stringify(path))
      ...
      if(taskname.search(deflt.cucoRE) === 0){
        ...
        if(noOfCalibs > 0){
          for(var i = 0; i < noOfCalibs; i++){
            ...
            log.info({ok:true}, "replaced CuCo by " + devicename)
