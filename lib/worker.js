...
var noderelay = function(mp, task, path, cb){
  ...
  request.exec(con, task, path,  wrtdata, function(data){
            ...
            mp.definition.del([pos], function(){
                mp.recipe.del([pos], function(){
                  ...
                  mp.definition.set([pos], def.Definition, function(){
                    walk.setstate(mp, pos, def.Definition, ctrlstr.ready, function(){
...
var getList = function(mp, task, path, cb){
  ...
  // (mp, con, task, path, wrtdata, cb)
  request.exec(con, task, path, false
              , function(listdata){
  ...
  mp.id.get([], function(sd){
    ...
    for(var i = 0; i < N; i++){
      ...
      log.info({ok:true}
                     ...
                     // (mp, con, task, path, wrtdata, cb)
                     request.exec(getcon, task, path, false
                                 , function(last, id){
                                     return function(updoc){
                                       if(updoc._id){
                                         ...
                                         log.info({ok:true}
                                                 ...
                                                 + id)
                                         // (mp, con, task, path, wrtdata, cb)
                                         request.exec(wrtcon, task, path,  wrtdata
