...
var getTime = function (task, cb){
  ...
  if(ep && !dp){
    ...
    utils.write_to_exchange(task,  timeobj, task.Path, cb);
  }
  ...
  if(ep && dp){
    ...
    utils.write_to_exchange(task,  timeobj, task.Path, function(){
      utils.query_cd(task,  {Result:[timeobj]}, cb);
    });
  }

  if(!ep && !dp){
    ro = {error:"missing value"};
    log.error(ro
...
var getDate = function (task, cb){
  ...
  if(ep && !dp){
    log.info(ro
            , "try to call write_to_exchange function");
    utils.write_to_exchange(task,  dateobj, task.Path, cb);
  }
  ...
  if(ep && dp){
    log.info(ro
            , "try to call write_to_exchange"
            + " function and query_cd function");
    utils.write_to_exchange(task,  dateobj, task.Path, function(){
      utils.query_cd(task, {Result:[ dateobj]}, cb)
    });
  }
...
var writeExchange = function (task, cb){
  ...
  if(val && exch && path && _.isArray(path) && _.isString(exch)){
    ...
    utils.write_to_exchange(task, val, task.Path, cb);
  }else{
    ro = {error:"not a valid task"};
    log.error(ro, "missing  ExchangePath or Value")
    if(_.isFunction (cb)){
      cb(ro);
    }
  }
...
var getList = function (task, cb){
  ...
  if(path && _.isArray(path) && exchpath && _.isString(exchpath)){
    ...
    request.exec(con, task, false, function(listdata){
      utils.write_to_exchange(task, listdata, task.Path, cb);
    });
  }else{
    ro = {error: "wrong path"}
    log.error(ro
...
var checkDB = function (task, cb){
  ...
  if(path && _.isArray(path) ){
    ...
    request.exec(con, task, false, function(dbinfo){
      ...
      utils.write_to_exchange(task, dbinfo, task.Path, cb);
  ...
  if(path && _.isArray(path) && exchpath && _.isString(exchpath)){
    ...
    request.exec(con, task, JSON.stringify(tsk), function (data){
      if(data.Result || data.error){
        ...
        utils.write_to_exchange(task, relayinfo, task.Path, cb);

      }else{
        err = new Error("receive unexpected data");
        log.error(err
