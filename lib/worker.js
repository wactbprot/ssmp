...
var select = function (task, cb){
  ...
  if(TaskDefClass){
      ...
      if(!err){
        ...
        for(var i = 0; i < dN; i++){
          ...
          if(definitions[i].DefinitionClass == TaskDefClass){
            var cN =  definitions[i].Condition.length;
            for(var j = 0; j < cN; j++){
                ...
                (function(k, l){
                  ...
                  if(condobj.Methode &&
                     compare[condobj.Methode] &&
                     condobj.Value &&
                     condobj.ExchangePath){
                    var e_path  = [mpid,"exchange"].concat(condobj.ExchangePath.split("."));
                    mem.get(e_path, function(err, exval){
                      ...
                      if(l == cN -1){
                        if(take){
                          ...
                          log.info(ro
                                  , "definition dont match conditions");
                        }
                      }
                    });
                  }else{
            ...
            log.error(ro
                     , "DefinitionClass " + definitions[i].DefinitionClass +" don't match" );
    ...
    ro = {error:"no definition class"}
    log.error(ro
             , "task contains no definition class key");
    if(_.isFunction(cb)){
      cb(ro);
    }
  }
