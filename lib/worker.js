...
var wait = function(task, cb){
  ...
  if(_.isString(wt)){
    ...
    log.warn({warn:"wait time is string"}
...
var writeExchange = function (task, cb){
  ...
  if(val && key && path && _.isArray(path) && _.isString(key)){
    var mpid    = path[0]
    ...
    mem.set([mpid, "exchange"].concat(key.split(".")), val, function (err){

      if(!err){
        log.info(ro,
                 "wrote " + key + " to Exchange")
        mem.publish("exchange", [mpid, "exchange", key], function (err){
          if(!err){
            log.info(ro,
...
var readExchange = function (task, cb){
  ...
  if(key && path && _.isArray(path)){
    ...
    mem.get([mpid, "exchange", key], function (err, data){
      if(!err){
        if(_.isUndefined(data)){
          ...
          // data.Ready gibt:
          if( _.isUndefined(dR) || dR == true || dR == "true"){
            log.info(ro
                    , "receive ready; analyse and collect data");
            ...
            if(!_.isEmpty(valcoll)){
              ...
              utils.query_cd(task, {Result:[valcoll]}
                            , function (res){
                                if(res.ok){
                                  if(_.isUndefined(data.Ready)){
                                    ...
                                    mem.set([mpid, "exchange", key, "Ready"], false, function (err){
                                      if(res.ok){
                                        log.info(ro
                                                , "reset exchange." + key + ".Ready to false" );
                                        mem.publish("exchange", [mpid,"exchange", key, "Ready"], function (err){
                                          log.info(ro
...
var getList = function (task, cb){
  ...
  if(path && _.isArray(path) && exchpath && _.isString(exchpath)){
    ...
    request.exec(con, task, false, function(listdata){
      mem.set([mpid, "exchange"].concat(exchpath.split(".")), listdata, function (err){
        if(!err){
          mem.publish("exchange", [mpid, "exchange"].concat(exchpath.split(".")), function (err){
            if(!err){
              log.info(ro
...
var checkDB = function (task, cb){
  ...
  if(path && _.isArray(path) && exchpath && _.isString(exchpath)){
    ...
    request.exec(con, task, false, function(dbinfo){
      ...
      mem.set([mpid, "exchange"].concat(exchpath.split(".")), dbinfo, function (err){
        if(!err){
          mem.publish("exchange", [mpid, "exchange"].concat(exchpath.split(".")), function (err){
            if(!err){
              log.info(ro
  ...
  if(path && _.isArray(path) && exchpath && _.isString(exchpath)){
    ...
    request.exec(con, task, JSON.stringify(tsk), function (data){
      if(data.Result || data.error){
        ...
        mem.set([mpid, "exchange"].concat(exchpath.split(".")), relayinfo , function (err){
          if(!err){
            mem.publish("exchange", [mpid, "exchange"].concat(exchpath.split(".")), function (err){
              if(!err){
                log.info(ro
            ...
            request.exec(net.list(task), task, false, function (last, id){
                                                return function (updoc){
                                                  if(updoc._id){
                                                    ...
                                                    request.exec(wrtcon, task, wrtdata, function (res){
                                                      if(res.ok){
                                                        log.info(ro
