...
var noderelay = function(mp, task, path, cb){
  ...
  log.info(ro,
           "try relay request");
  request.exec(mp,  task, path, con, wrtdata, false, cb)
};
  ...
  if(task.Value &&
    ...
    for(var i = 0; i < rcps.length; i++){
      ...
      if(rcp.RecipeClass === rclass){
        ...
        if(take){
          ro = {ok:true};
          log.info(ro, "found matching recipe")
...
var getList = function(mp, task, path, cb){
  ...
  var con     = net.list(mp, task);

  request.exec(mp,  task, path, con, false, false, cb)
...
var updateCd = function(mp, task, path, cb){
  ...
  for(var i = 0; i < N; i++){
    ...
    request.exec(mp,  task, path, getcon, false, false,
                 function(last, id){
                   return function(updoc){
                            if(updoc._id){
                              var wrtcon    = net.wrtdoc(mp, id)
                                , wrtdata   = JSON.stringify(updoc);
                              log.info({ok:true}
                                      ,"try to write doc with id: "
                                      + id)

                              request.exec(mp,  task, path, wrtcon, wrtdata, false,
                                           function(res){
                                             if(res.ok){
                                               log.info({ok:true}
                                                       ,"doc with id: "
