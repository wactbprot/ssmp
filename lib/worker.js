var _        = require("underscore")
  ...
  , log      = bunyan.createLogger({name: deflt.app.name + ".worker"})
    ...
    mem.get([mpid, "definitions"], function(err, definitions){
      if(!err && definitions && _.isArray(definitions)){
        ...
        for(var i = 0; i < Ndef; i++){
          if(stop) break;
          if(definitions[i].DefinitionClass == TaskDefClass && definitions[i].Condition){
            var Ncond = definitions[i].Condition.length
            log.info(ro
                    , "found matching DefinitionClass investigate "
                    + Ncond + " conditions");
            for(var j = 0; j <  Ncond; j++){
              if(stop) break;
              (function(k, l){
                ...
                if(condobj.Methode && compare[condobj.Methode] && !_.isUndefined(condobj.Value) && condobj.ExchangePath){
                  ...
                  mem.get([mpid,"exchange"].concat(condobj.ExchangePath.split(".")), function(err, exval){
                    if(!err && !_.isUndefined(exval)){
                      ...
                      last = l == definitions[k].Condition.length -1;
                      ...
                      log.error(err
                               , "on attempt to get exchange Value from: "
